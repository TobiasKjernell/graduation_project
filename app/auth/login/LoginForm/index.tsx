...
const LoginForm = () => {
    const { register, handleSubmit, formState: { errors }, reset } = useForm({ resolver: zodResolver(loginSchema), mode: 'onChange' });
    const { mutate, data, isPending } = useMutation({ mutationFn: LogIn })
    ...
    return (
        <form onSubmit={handleSubmit(values => mutate((values)))} className="p-4 flex flex-col w-full md:w-[400px] mx-auto bg-neutral-900 rounded-2xl border" >
            ...
            <div className='flex flex-col items-center justify-center gap-2'>
                <button className=" m-auto mt-5 cursor-pointer bg-neutral-800 rounded-xl p-3 px-5 border hover:bg-neutral-900">{isPending ? <Spinner /> : 'Login'}</button>
