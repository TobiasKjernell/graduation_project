...
const CreatePostForm = ({ handleOnClick, user }: { handleOnClick: () => void, user: string }) => {
    const { register, handleSubmit, reset, watch, formState: { errors } } = useForm({ resolver: zodResolver(blogPostSchema) });
    ...
    return <div className="fixed h-full overflow-y-scroll overflow-x-hidde w-screen flex flex-col psp-linear-background top-0 b-0 z-10 psp-text-jura items-center">
        ...
        <form className="max-w-[80%] w-full flex flex-col gap-2">
