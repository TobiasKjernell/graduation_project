...
const DashboardKanbanCard = ({ status, options, post }: IKanbanCard) => {
    const { toggleEditing, handleSetTicket } = useTicketEdit()
    ...
    return (
        <div className={`${post.assigned + "-" + post.id.toString()} p-2 border m-2 flex flex-col gap-2 bg-zinc-900 rounded-sm`}>
            <div className="flex gap-2">
                ...
                <button onClick={() => { handleSetTicket(post); toggleEditing(); }} className="cursor-pointer">
